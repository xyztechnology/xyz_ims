...
namespace InventroryManagementSystem.Controllers
{
    public class ProductCategoryController : Controller
    {
        ...
        public List<FolderTreeModel.FolderData> GetFolderName()
        {
            ...
            {
                var CountFile = db.Product.Where(x => x.CategoryId == item.CategoryId).Count();
                list.Add(new FolderTreeModel.FolderData() { FolderId = item.CategoryId, ParentFolderId = item.ParentCategoryId, FolderName = item.Name + "(" + CountFile + ")" });
            }
